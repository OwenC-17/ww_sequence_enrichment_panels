...
ggplot(filter(merged_rpip_reports_full, D == "Viruses"), 
...
ggplot(filter(merged_rpip_reports_full, D == "Viruses" & !startsWith(F, "unidentified") & RA > 0.0001), 
...
ggplot(filter(merged_rpip_reports_full, D == "Viruses" & Treatment != "FA" & RA > 0.000001), 
       aes(x = LIMS_ID, y = nodeOnly, fill = F)) + 
  geom_bar(position = 'stack', stat = 'identity') +
  ylab("Number of reads") +
  xlab("Sample No.") +
...
ggplot(filter(merged_rpip_reports_full, F == "Polyomaviridae"), #Plot all polyomaviruses 
...
ggplot(filter(merged_rpip_reports_full, F == "Coronaviridae"), #Plot all coronaviruses 
       aes(x = LIMS_ID, y = nodeOnly, fill = S)) + 
  geom_bar(position = 'stack', stat = 'identity') +
  ylab("Number of reads") +
  xlab("Sample No.") +
...
ggplot(filter(merged_rpip_reports_full, F == "Astroviridae"), #Plot all polyomaviruses 
       aes(x = LIMS_ID, y = nodeOnly, fill = S)) + 
  geom_bar(position = 'stack', stat = 'identity') +
  ylab("Number of reads") +
  xlab("Sample No.") +
...
virus_counts <- merged_rpip_reports %>%
  filter(D == "Viruses") %>%
  group_by(SampleID) %>%
  summarise(sum = sum(nodeOnly), ra_sum = sum(RA))

sample_34970 <- filter(merged_rpip_reports_full, LIMS_ID == 34970)

retentate_34970 <- filter(sample_34970, fraction == "Retentate")
