...
k2_vsp_main_dir <- paste0("input/link_to_raw_data/vsp_panels/raw_fastqs/",
...
if(exists("merged_vsp_reports_full")) {rm(merged_vsp_reports_full)}
for (data in list.files(k2_vsp_main_dir,
                        pattern = "*.tsv",
                        full.names = TRUE)) { 
  
  # Create the initial df if none exists yet
  if (!exists("merged_vsp_reports_full")) { 
    merged_vsp_reports_full <- generate_tax_table(data) %>% 
      fill_tax_NAs() %>% 
  ...
  if (exists("merged_vsp_reports_full")) {
    temporary <- generate_tax_table(data) %>% #temporary table is necessary so 
      ...
      fill_tax_NAs() %>%
      mutate(SampleID = basename(data))
    merged_vsp_reports_full <- bind_rows(merged_vsp_reports_full, temporary)
...
write_rds(x = merged_vsp_reports_full,
          file = "input/modified/k2_vsp_reports_full.rds")
