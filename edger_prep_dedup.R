...
saved_dedup_reports_path <- paste0("imported_k2_reports/",
                                   "all_k2_reports_anyConf_dedup.csv")
...
###Deduped versions###
all_k2_reports_deduped <- read_csv(saved_dedup_reports_path, guess_max = Inf)
all_k2_reports_deduped <- assert_k2_report_coltypes(all_k2_reports_deduped)
all_k2_reports_deduped <- parse_sample_treatments(all_k2_reports_deduped)
######################
...
allFamilies_anyConf_noRrna_dedup <- collapse_to_family_no_rrna(all_k2_reports_deduped)
...
allFamilies_anyConf_withRrna_dedup <- collapse_to_family_keep_rrna(
...
allFamilies_anyConf_noRrna_dedup <- allFamilies_anyConf_noRrna_dedup %>%
  mutate(UniqueID = paste(LIMS_ID, Treatment, Enrichment, Kraken2_confidence, sep = "-"))
...
allFamilies_anyConf_withRrna_dedup <- allFamilies_anyConf_withRrna_dedup %>%
  mutate(UniqueID = paste(LIMS_ID, Treatment, Enrichment, Kraken2_confidence, sep = "-"))
...
family_count_matrix_noRrna_dedup <- allFamilies_anyConf_noRrna_dedup %>%
  pivot_wider(id_cols = !!sym("F"),
              names_from = UniqueID,
              values_from = readcount,
              values_fill = 0)
...
family_count_matrix_withRrna_dedup <- allFamilies_anyConf_withRrna_dedup %>%
  pivot_wider(id_cols = !!sym("F"),
              names_from = UniqueID,
              values_from = readcount,
              values_fill = 0)
...
write_csv(sample_metadata_noRrna, 
