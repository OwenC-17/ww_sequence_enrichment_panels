...
#"merged_pre_reports_full.csv"; do not run the import loop again unless the
#original files change, as it will take hours

#This will merge several kraken2 reports into one df
rm(merged_pre_reports_full)
for (data in list.files()){ #List files in current directory as "data"
  
  # Create the initial df if none exists yet
  if (!exists("merged_pre_reports_full")){ 
    merged_pre_reports_full <- generate_tax_table(data) %>% #generate_tax_table() is 
  ...
  if (exists("merged_pre_reports_full")){
    ...
    merged_pre_reports_full <- bind_rows(merged_pre_reports_full, temporary) #dplyr row 
...
write_csv(x = merged_pre_reports_full, file = "merged_pre_reports_full.csv")
