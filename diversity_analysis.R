...
h_ <- freqs %>%
  pivot_wider(names_from = nucleotide, values_from = c(freq, countfr), 
              values_fill = 0) %>%
  dplyr::rename(freq_Ins = `freq_+`, freq_Del = `freq_-`, countfr_Ins = `countfr_+`, countfr_Del = `countfr_-`) %>%
  mutate(total_nts = rowSums(across(c(freq_A, freq_C, freq_G, freq_T, freq_Ins, freq_Del), ~ .  > 0)))
h_ <- h_ %>%
  mutate(n = countfr_Ins + countfr_Del + countfr_A + countfr_C + countfr_G + countfr_T,
         h = case_when(n > 1 ~ ((n  / (n - 1))*(1 - freq_A^2 - freq_C^2 - freq_G^2 - freq_T^2 - freq_Ins^2 - freq_Del^2)),
                       .default = 0))
...
hsumpi_ <- h_ %>%
  ...
  summarise(num_covsites = n(),
            pi = sum(h),
            persitepi = pi/n())
ggplot(hsumpi_, aes(x = pi, y = persitepi)) + geom_point() + scale_x_log10() + scale_y_log10()
